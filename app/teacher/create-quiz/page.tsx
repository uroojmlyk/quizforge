...
import {
  PlusCircle, Trash2, ArrowLeft, Save, Globe, Users,
  ChevronDown, ChevronUp, X, GraduationCap, CheckCircle,
  HelpCircle, FileText, Clock, Star, Sparkles, Loader2, Lock
} from 'lucide-react';
import toast from 'react-hot-toast';
import { showToast } from '@/lib/toast';
...
function focusBorder(e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) {
  (e.target as HTMLElement).style.borderColor = T.borderFocus;
  (e.target as HTMLElement).style.boxShadow = `0 0 0 3px ${T.accentBg}`;
}
...
export default function CreateQuizPage() {
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      ...
      if (data.success && data.questions?.length > 0) {
        ...
        setShowAiPanel(false);
        setAiTopic('');
      } else { showToast.error(data.error || 'Failed to generate questions'); }
  ...
  const toggleStudent = (id: string) => {
    setAssignedStudents(prev => prev.includes(id) ? prev.filter(s => s !== id) : [...prev, id]);
  };
  const selectAllStudents = () => setAssignedStudents(filteredStudents.map(s => s._id));
  const clearAllStudents = () => setAssignedStudents([]);
  ...
  const validateQuiz = () => {
    ...
    for (let i = 0; i < questions.length; i++) {
      if (!questions[i].text.trim()) { showToast.error(`Question ${i + 1} is empty`); return false; }
      for (let j = 0; j < questions[i].options.length; j++) {
        if (!questions[i].options[j].trim()) { showToast.error(`Option ${j + 1} of Q${i + 1} is empty`); return false; }
      }
    }
    if (visibility === 'assigned' && assignedStudents.length === 0) {
      showToast.error('Please select at least one student'); return false;
  ...
  const visibilityOptions = [
    {
      ...
      label: 'Public',
      desc: 'Visible to all students',
      icon: Globe,
      color: T.accent,
      bg: T.accentBg,
      border: T.accentBorder,
      activeBg: 'rgba(52,211,153,0.06)',
      activeBorder: T.accentBorder,
    },
    {
      value: 'assigned' as const,
      label: 'Assigned Only',
      desc: 'Selected students only',
      icon: Users,
      color: '#f59e0b',
  ...
  return (
    <div className="min-h-screen text-white overflow-x-hidden"
      style={{ background: T.bg, fontFamily: "'DM Sans','Inter',sans-serif" }}>
        ...
        <div className="max-w-4xl mx-auto px-4 h-14 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <button onClick={() => router.back()}
              className="p-2 rounded-xl transition-all hover:bg-white/[0.04] shrink-0"
              style={{ color: T.muted }}>
              <ArrowLeft className="w-4 h-4" />
            </button>
            {/* QuizForge logo */}
            <div className="flex items-center gap-2 shrink-0">
              ...
              <span className="font-semibold text-sm hidden sm:block"
                style={{ color: 'rgba(255,255,255,0.8)' }}>QuizForge</span>
            </div>
            <div className="w-px h-4 shrink-0" style={{ background: T.border }} />
            <div className="min-w-0">
              <p className="text-sm font-semibold text-white leading-none">Create Quiz</p>
              <p className="text-[10px] mt-0.5 hidden sm:block" style={{ color: T.dim }}>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-xl border"
              ...
              <HelpCircle className="w-3.5 h-3.5" style={{ color: T.dim }} />
              <span className="text-xs" style={{ color: T.muted }}>
                {questions.length} Q · {totalMarks} marks · {duration}m
              </span>
            </div>
      ...
      {/* ── Main ── */}
      <div className="relative z-10 max-w-4xl mx-auto px-4 py-6 pb-12 space-y-4">

        {/* Page header */}
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-white"
            style={{ fontFamily: "'DM Serif Display', serif" }}>
            New Quiz<span style={{ color: T.accent }}>.</span>
          </h1>
          ...
          <div className="p-4 space-y-3.5">
            ...
            <div>
              ...
              <textarea value={description} onChange={e => setDescription(e.target.value)}
                rows={3} placeholder="Brief description of what this quiz covers..."
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" value={duration} onChange={e => setDuration(Number(e.target.value))} min="1"
                    className="w-32 pl-9 pr-3 py-2.5 rounded-xl text-sm focus:outline-none transition-all"
                    style={inputBase} onFocus={focusBorder} onBlur={blurBorder} />
                </div>
                <p className="text-[10px] mt-1" style={{ color: T.dim }}>minutes</p>
              </div>
        ...
        <div className="rounded-2xl overflow-hidden border" style={{ background: T.card, borderColor: T.border }}>
          ...
          <div className="p-4 space-y-2.5">
              ...
              return (
                <label key={opt.value}
                  ...
                  style={{
                    ...
                    borderColor: isActive ? opt.activeBorder : T.border,
                  }}
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-xl flex items-center justify-center"
                      style={{ background: isActive ? opt.bg : 'rgba(255,255,255,0.04)' }}>
                      <opt.icon className="w-4 h-4"
                        style={{ color: isActive ? opt.color : T.dim }} />
                    </div>
                    <div>
                      <p className="text-sm font-medium text-white">{opt.label}</p>
                      <p className="text-[11px]" style={{ color: T.muted }}>{opt.desc}</p>
                    </div>
                  </div>
                  {/* Custom radio */}
                  ...
                  <input type="radio" name="visibility" checked={isActive}
                    onChange={() => { setVisibility(opt.value); if (opt.value === 'assigned') setShowStudentSelector(true); }}
                    className="hidden" />
                </label>
              );
            })}
            ...
            {/* Student selector */}
            {visibility === 'assigned' && (
              <div className="mt-1 pt-4 border-t space-y-3" style={{ borderColor: T.border }}>
                <div className="flex items-center justify-between">
                  <button onClick={() => setShowStudentSelector(!showStudentSelector)}
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    <button onClick={selectAllStudents} className="transition-colors" style={{ color: T.accent }}
                    ...
                    <button onClick={clearAllStudents} className="transition-colors" style={{ color: '#f87171' }}
                ...
                {/* Selected chips */}
                {assignedStudents.length > 0 && (
                  <div className="p-3 rounded-xl border" style={{ background: 'rgba(255,255,255,0.02)', borderColor: T.border }}>
                    <p className="text-[11px] mb-2.5" style={{ color: T.dim }}>
                      ...
                      {assignedStudents.map(id => {
                        ...
                        return s ? (
                          <span key={id}
                            ...
                            style={{ background: 'rgba(245,158,11,0.08)', borderColor: 'rgba(245,158,11,0.2)', color: '#f59e0b' }}>
                            {s.name}
                            <X className="w-3 h-3 cursor-pointer" onClick={() => toggleStudent(id)} />
                          </span>
                        ) : null;
                      })}
                    </div>
                ...
                {showStudentSelector && (
                  <div className="rounded-xl overflow-hidden border" style={{ borderColor: T.border }}>
                    {/* Search */}
                    <div className="p-2.5 border-b relative" style={{ borderColor: T.border }}>
                      <input type="text" value={studentSearch}
                        onChange={e => setStudentSearch(e.target.value)}
                        placeholder="Search students..."
                        className="w-full px-3 py-2 pr-8 rounded-xl text-sm focus:outline-none transition-all"
                        style={inputBase} onFocus={focusBorder} onBlur={blurBorder} />
                      {studentSearch && (
                        <button onClick={() => setStudentSearch('')}
                          className="absolute right-5 top-1/2 -translate-y-1/2" style={{ color: T.muted }}>
                          <X className="w-3.5 h-3.5" />
                        </button>
                      )}
                    </div>
                    ...
                    <div className="max-h-52 overflow-y-auto">
                      {filteredStudents.length === 0 ? (
                        <div className="p-5 text-center">
                          <p className="text-xs" style={{ color: T.muted }}>No students found</p>
                        </div>
                      ) : filteredStudents.map(student => {
                        const isSelected = assignedStudents.includes(student._id);
                        return (
                          <div key={student._id} onClick={() => toggleStudent(student._id)}
                            className="flex items-center justify-between px-4 py-3 border-b cursor-pointer select-none transition-colors"
                            style={{
                            ...
                            <div className="min-w-0">
                              <p className="text-sm truncate" style={{ color: 'rgba(255,255,255,0.85)' }}>{student.name}</p>
                              <p className="text-xs truncate" style={{ color: T.dim }}>{student.email}</p>
                            </div>
                            {/* Checkbox */}
                            <div className="w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 ml-3 transition-all"
                              style={{
                    ...
                    {/* Footer */}
                    <div className="px-4 py-2.5 border-t" style={{ borderColor: T.border, background: 'rgba(255,255,255,0.01)' }}>
                      <p className="text-[10px] text-center" style={{ color: T.dim }}>
                        {filteredStudents.length} of {students.length} students · {assignedStudents.length} selected
                      </p>
                    </div>
                  </div>
        ...
        <div className="rounded-2xl overflow-hidden border" style={{ background: T.card, borderColor: T.border }}>
          <div className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: T.border }}>
            ...
            <div className="flex items-center gap-2">
              {/* AI button — uses consistent purple-ish tint but within design system */}
              <button onClick={() => setShowAiPanel(!showAiPanel)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all"
                style={showAiPanel
                  ? { background: 'rgba(167,139,250,0.15)', borderColor: 'rgba(167,139,250,0.35)', color: '#c4b5fd' }
                  : { background: 'rgba(167,139,250,0.08)', borderColor: 'rgba(167,139,250,0.2)', color: '#a78bfa' }}>
                <Sparkles className="w-3.5 h-3.5" />
                AI Generate
              </button>
          ...
          {showAiPanel && (
            <div className="m-4 p-4 rounded-2xl border"
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="text-sm font-semibold" style={{ color: '#c4b5fd' }}>AI Question Generator</p>
                  <p className="text-[11px]" style={{ color: T.muted }}>Powered by Groq · Free · Any topic</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input type="text" value={aiTopic} onChange={e => setAiTopic(e.target.value)}
                    ...
                    placeholder="e.g. Photosynthesis, World War II, Python loops..."
                    className="w-full px-3.5 py-2.5 rounded-xl text-sm focus:outline-none transition-all"
                    style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(167,139,250,0.2)', color: 'rgba(255,255,255,0.92)' }}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select value={aiCount} onChange={e => setAiCount(Number(e.target.value))}
                      className="w-full px-3 py-2.5 rounded-xl text-sm focus:outline-none appearance-none cursor-pointer"
                      style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(167,139,250,0.2)', color: 'rgba(255,255,255,0.92)' }}>
                      {[3, 5, 8, 10, 15].map(n => (
                        <option key={n} value={n} style={{ background: '#0f1012' }}>{n} questions</option>
                      ))}
                  ...
                  <div>
                    ...
                    <select value={aiDifficulty} onChange={e => setAiDifficulty(e.target.value as 'easy' | 'medium' | 'hard')}
                      className="w-full px-3 py-2.5 rounded-xl text-sm focus:outline-none appearance-none cursor-pointer"
                      style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(167,139,250,0.2)', color: 'rgba(255,255,255,0.92)' }}>
                      <option value="easy" style={{ background: '#0f1012' }}>Easy</option>
                      <option value="medium" style={{ background: '#0f1012' }}>Medium</option>
                ...
                <div className="flex items-center gap-3 pt-1">
                  <button onClick={handleAiGenerate} disabled={aiLoading || !aiTopic.trim()}
                    className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold border transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                    style={{ background: 'rgba(167,139,250,0.15)', borderColor: 'rgba(167,139,250,0.3)', color: '#c4b5fd' }}>
                    {aiLoading
                      ? <><Loader2 className="w-4 h-4 animate-spin" /> Generating...</>
                      : <><Sparkles className="w-4 h-4" /> Generate Questions</>}
                  </button>
                  <button onClick={() => { setShowAiPanel(false); setAiTopic(''); }}
                    className="text-xs transition-colors" style={{ color: T.muted }}
          ...
          <div className="divide-y" style={{ borderColor: T.border }}>
            {questions.map((q, qIndex) => (
              <div key={q.id} className="p-4 space-y-3.5">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-lg flex items-center justify-center shrink-0"
                      style={{ background: T.accentBg, border: `1px solid ${T.accentBorder}` }}>
                      <span className="text-[10px] font-bold" style={{ color: T.accent }}>{qIndex + 1}</span>
                    </div>
                    <span className="text-xs" style={{ color: T.dim }}>Question {qIndex + 1}</span>
                  </div>
                  {questions.length > 1 && (
                ...
                {/* Question text */}
                <textarea value={q.text} onChange={e => updateQuestion(qIndex, 'text', e.target.value)}
                  rows={2} placeholder={`Type question ${qIndex + 1} here...`}
                  className="w-full px-3.5 py-2.5 rounded-xl text-sm focus:outline-none transition-all resize-none"
                  style={inputBase} onFocus={focusBorder} onBlur={blurBorder} />
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <div>
                    <Label>Marks</Label>
                    <div className="relative">
                      <Star className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5" style={{ color: T.dim }} />
                      <input type="number" value={q.marks} min="1"
                        onChange={e => updateQuestion(qIndex, 'marks', Number(e.target.value))}
                        className="w-full sm:w-24 pl-9 pr-3 py-2.5 rounded-xl text-sm focus:outline-none transition-all"
                        style={inputBase} onFocus={focusBorder} onBlur={blurBorder} />
                    </div>
                  </div>
                </div>
          ...
          <div className="p-4 border-t" style={{ borderColor: T.border }}>
            <button onClick={addQuestion}
              ...
              onMouseEnter={e => {
                ...
                (e.currentTarget as HTMLElement).style.background = T.accentBg;
              }}
              onMouseLeave={e => {
        ...
        <div className="rounded-2xl p-4 border flex items-center justify-between"
          ...
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: T.muted }}>
              {totalMarks} marks · {duration} min · {visibility}
            </p>
          </div>
          <button onClick={handleSubmit} disabled={saving}
            ...
            onMouseEnter={e => !saving && ((e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.9)')}
