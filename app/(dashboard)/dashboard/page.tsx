  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: T.bg }}>
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: T.accentBg, border: `1px solid ${T.accentBorder}` }}>
            {/* QuizForge Q logo mark */}
      ...
      <nav className="sticky top-0 z-50 border-b backdrop-blur-xl" style={{ background: T.navBg, borderColor: T.border }}>
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            <span className="font-semibold text-sm hidden sm:block" style={{ color: 'rgba(255,255,255,0.85)' }}>
              QuizForge
            </span>
          </div>
          ...
          <AnimatePresence>
            {showSearch && (
              <motion.div initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: '100%' }}
                exit={{ opacity: 0, width: 0 }} className="flex-1 max-w-sm">
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5" style={{ color: T.muted }} />
                  <input autoFocus type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                    placeholder="Search quizzes..."
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <div className="relative">
              <button onClick={() => setNotifOpen(!notifOpen)}
                className="p-2 rounded-xl transition-all hover:bg-white/[0.04] relative" style={{ color: T.muted }}>
                <Bell className="w-4 h-4" />
                {availableQuizzes.length > 0 && (
                  <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 rounded-full" style={{ background: T.accent }} />
                )}
            ...
            {/* Profile */}
            <Link href="/profile">
              <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-xl transition-all cursor-pointer border ml-1"
                style={{ background: T.card, borderColor: T.border }}>
                <div className="w-5 h-5 rounded-md flex items-center justify-center text-white text-[10px] font-bold shrink-0"
                  style={{ background: 'rgba(52,211,153,0.2)', border: '1px solid rgba(52,211,153,0.3)' }}>
                  {user?.name?.charAt(0)?.toUpperCase() || 'S'}
                </div>
                <span className="text-xs hidden sm:block max-w-[80px] truncate" style={{ color: T.muted }}>
                  {user?.name}
                </span>
              </div>
            </Link>
            ...
            <button onClick={handleLogout}
              className="p-2 rounded-xl transition-all hover:bg-red-500/[0.08] ml-1" style={{ color: T.muted }}>
              <LogOut className="w-4 h-4" />
            </button>
          </div>
        </div>
            ...
            <div className="flex items-center gap-3 rounded-2xl px-4 py-2.5 border"
              ...
              <div className="relative w-10 h-10">
                <svg className="w-10 h-10 -rotate-90" viewBox="0 0 44 44">
                  <circle cx="22" cy="22" r="18" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="3" />
                  <circle cx="22" cy="22" r="18" fill="none" stroke={T.accent} strokeWidth="3"
                    strokeDasharray={`${Math.min(100, avgScore) * 1.13} 113`} strokeLinecap="round"
                    style={{ opacity: 0.7 }} />
                </svg>
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-[10px] font-bold text-white">{avgScore}%</span>
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          ...
          <div className={`lg:col-span-2 space-y-4 ${activeTab !== 'quizzes' ? 'hidden lg:block' : ''}`}>
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
                ...
                {searchQuery && (
                  <div className="flex items-center gap-2 mt-2">
                    <p className="text-xs" style={{ color: T.muted }}>
                      Results for <span style={{ color: T.accent }}>"{searchQuery}"</span>
                    </p>
              ...
              {filteredQuizzes.length === 0 ? (
                <div className="p-10 text-center">
                  ...
                  <p className="text-sm" style={{ color: T.muted }}>
                    {searchQuery ? 'No quizzes match' : 'No quizzes available'}
                  </p>
                  {activeFilter !== 'all' && (
                    <button onClick={() => setActiveFilter('all')} className="mt-2 text-xs" style={{ color: T.accent }}>
                      View all quizzes
                ...
                <div className="divide-y" style={{ borderColor: T.border }}>
                  {filteredQuizzes.slice(0, 8).map((quiz, i) => (
                    <motion.div key={quiz.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                      ...
                      onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                      <div className="flex items-center gap-3">
                        <div className="hidden sm:flex w-9 h-9 rounded-xl border items-center justify-center shrink-0"
                          style={{ background: 'rgba(255,255,255,0.03)', borderColor: T.border }}>
                          {quiz.visibility === 'assigned'
                            ? <Lock className="w-4 h-4" style={{ color: '#f59e0b' }} />
                            : <BookOpen className="w-4 h-4" style={{ color: T.accent }} />}
              ...
              {filteredQuizzes.length > 8 && (
                <div className="p-3.5 border-t" style={{ borderColor: T.border }}>
                  <Link href="/quizzes" className="flex items-center justify-center gap-2 text-xs transition-colors"
                    style={{ color: T.muted }}>
                    View {filteredQuizzes.length - 8} more <ArrowRight className="w-3 h-3" />
                  </Link>
                </div>
            ...
            {results.length > 0 && (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                ...
                <div className="flex items-center justify-between px-4 py-3.5 border-b" style={{ borderColor: T.border }}>
                  ...
                  <Link href="/results" className="text-xs flex items-center gap-1 transition-colors"
                    style={{ color: T.muted }}
                    onMouseEnter={e => (e.currentTarget.style.color = T.accent)}
                    onMouseLeave={e => (e.currentTarget.style.color = T.muted)}>
                    All results <ChevronRight className="w-3 h-3" />
          ...
          {/* ── Right sidebar ── */}
          <div className={`space-y-4 ${activeTab !== 'quizzes' && activeTab !== 'results' ? '' : activeTab === 'quizzes' ? 'hidden lg:block' : activeTab === 'results' ? 'hidden lg:block' : ''}`}>
            <div className="hidden lg:block space-y-4">
              <SidebarContent results={results} avgScore={avgScore} totalPoints={totalPoints} streak={streak} T={T} />
            </div>
            <div className="lg:hidden">
              <SidebarContent results={results} avgScore={avgScore} totalPoints={totalPoints} streak={streak} T={T} />
            </div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.38 }}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-white mb-1">Keep going!</p>
              <p className="text-xs" style={{ color: T.muted }}>
                You've completed {results.length} quiz{results.length !== 1 ? 'zes' : ''}. Next milestone at {Math.ceil(results.length / 5) * 5}.
              </p>
              <div className="mt-3 h-1 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.08)' }}>
                <div className="h-full rounded-full transition-all duration-700"
                  style={{ width: `${(results.length % 5) * 20}%`, background: T.accent, opacity: 0.7 }} />
